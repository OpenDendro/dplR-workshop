...
Please install `R` by visiting [www.r-project.org](https://www.r-project.org/). 
We recommend that you use [RStudio](https://www.rstudio.com/) to interact with,
and script in `R`. These documents were all made using `r version$version.string`.

### Getting dplR

Install the add-on library `dplR`. You can download and install these 
using the `install.packages` function from the `R` prompt:
```{r eval=FALSE}
...
No prior `R` experience is necessary but for those who are new to R, we suggest using the resources at [swirl](http://swirlstats.com/) as a way of starting to learn `R`.
